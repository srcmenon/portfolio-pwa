export default async function handler(req, res) {
  ...
  const YahooFinance = yf2module.default
  ...
  for (let i = 0; i < positions.length; i += BATCH) {
    ...
    await Promise.all(batch.map(async pos => {
      ...
      const f = await fetchFundamentals(yahooFinance, ticker)
...
function toYahooTicker(pos) {
  ...
  const map = { SEMI:"CHIP.PA", EWG2:"EWG2.SG", DFNS:"DFNS.L", IWDA:"IWDA.L", EIMI:"EIMI.L", SSLV:"SSLV.L", SGLN:"SGLN.L" }
  return map[t] || t
}
...
function scoreGoalAlignment(pos, sector, goals) {
  ...
  if (pos.currency === "INR") {
    ...
    const good = ["bank","financial","nbfc","software","it","technology","pharma","healthcare","consumer","fmcg","capital goods","industrial","machinery","chemicals","ratings","analytics","food"]
...
function getVerdict(techScore, techVerdict, fundScore, fundHasData, goalScore, pos) {
  ...
  if (fundHasData) {
    if (fundScore<30)                   { verdict="EXIT"; priority="HIGH";   reasoning="Poor fundamentals confirm exit"; action=`Sell all ${pos.qty||""} shares — business quality insufficient for long-term hold.` }
    ...
    else if (isSell && fundScore>=60)   { verdict="HOLD"; priority="MEDIUM"; reasoning="Weak technicals but strong fundamentals — temporary dip in quality business"; action=`Hold. Fundamentals contradict sell signal. Add if RSI drops below 35.` }
    ...
    else                                { verdict="HOLD"; priority="LOW";    reasoning="Consolidating with solid fundamentals"; action=`Hold — quality business in consolidation. Add if RSI drops below 42.` }
  } else {
    if (isBuy)  { verdict="ADD";    priority="MEDIUM"; reasoning="Technical BUY — no fundamental data available"; const q=Math.max(1,Math.round(5000/(cur||1))); action=pos.currency==="INR"?`Add ${q} shares at ₹${cur.toFixed(0)} — verify on Screener.in first`:`Add €150 — verify fundamentals first` }
    else if (isSell) { verdict="REVIEW"; priority="MEDIUM"; reasoning="Technical SELL — verify on Screener.in before exiting"; action=`Check Screener.in for ${pos.key} fundamentals before exiting.` }
    else        { verdict="HOLD";   priority="LOW";    reasoning="Neutral — no fundamental data available"; action=`Hold. Verify fundamentals on Screener.in.` }
  }
